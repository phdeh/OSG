...
DWORD WINAPI thread_b(LPVOID lpParam) {
    ...
    ReleaseSemaphore(semaphore_b, 9, NULL);
    return TRUE;
}
...
DWORD WINAPI thread_g(LPVOID lpParam) {
    ...
    WaitForMultipleObjects(5, semaphores, TRUE, 0L);
    for (int i = 0; i < 8; i++) {
        WaitForSingleObject(lock, 0L);
...
DWORD WINAPI thread_h(LPVOID lpParam) {
    ...
    ReleaseSemaphore(semaphore_h, 4, NULL);
    return TRUE;
}
DWORD WINAPI thread_k(LPVOID lpParam) {
    ...
    WaitForMultipleObjects(7, semaphores, TRUE, 0L);
    for (int i = 0; i < 8; i++) {
        WaitForSingleObject(lock, 0L);
...
DWORD WINAPI thread_p(LPVOID lpParam) {
    ...
    ReleaseSemaphore(semaphore_p, 1, NULL);
    return TRUE;
}
int lab3_init() {
    DWORD ThreadID;
