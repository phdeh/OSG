...
DWORD WINAPI thread_a(LPVOID lpParam) {
    ...
    ReleaseSemaphore(semaphore_a, 13, NULL);
    return TRUE;
}
DWORD WINAPI thread_b(LPVOID lpParam) {
    ...
    ReleaseSemaphore(semaphore_b, 10, NULL);
    return TRUE;
}
...
DWORD WINAPI thread_d(LPVOID lpParam) {
    ...
    ReleaseSemaphore(semaphore_d, 7, NULL);
    return TRUE;
}
...
DWORD WINAPI thread_h(LPVOID lpParam) {
    ...
    ReleaseSemaphore(semaphore_h, 5, NULL);
    return TRUE;
}
...
DWORD WINAPI thread_k(LPVOID lpParam) {
    ...
    WaitForMultipleObjects(9, semaphores, TRUE, 0L);
    for (int i = 0; i < 4; i++) {
        WaitForSingleObject(lock, 0L);
...
DWORD WINAPI thread_p(LPVOID lpParam) {
    ...
    WaitForMultipleObjects(11, semaphores, TRUE, 0L);
    for (int i = 0; i < 4; i++) {
        WaitForSingleObject(lock, 0L);
...
DWORD WINAPI thread_r(LPVOID lpParam) {
    ...
    ReleaseSemaphore(semaphore_r, 2, NULL);
    return TRUE;
}
int lab3_init() {
    DWORD ThreadID;
    ...
    aThread[13] = CreateThread( 
        NULL,       // default security attributes
        0,          // default stack size
        (LPTHREAD_START_ROUTINE) thread_r, 
        NULL,       // no thread function arguments
        0,          // default creation flags
        &ThreadID); // receive thread identifier
    if(aThread[13] == NULL) {
