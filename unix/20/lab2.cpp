...
int lab2_init() {
    ...
    if ((semaphore_i = sem_open("/my_semaphore_i", O_CREAT, 0777, 0)) == SEM_FAILED) {
        std::cerr << "Semaphore I init failed" << std::endl;
        return 1;
    }
    if ((semaphore_k = sem_open("/my_semaphore_k", O_CREAT, 0777, 0)) == SEM_FAILED) {
        std::cerr << "Semaphore K init failed" << std::endl;
        return 1;
    }
    if ((semaphore_m = sem_open("/my_semaphore_m", O_CREAT, 0777, 0)) == SEM_FAILED) {
        std::cerr << "Semaphore M init failed" << std::endl;
        return 1;
    }
    if ((semaphore_r = sem_open("/my_semaphore_r", O_CREAT, 0777, 0)) == SEM_FAILED) {
        std::cerr << "Semaphore R init failed" << std::endl;
        return 1;
    }
    int err;
    ...
    err = pthread_create(&thread_id_n, NULL, thread_n, (void *) NULL);
    if (err != 0)
        std::cerr << "Can't create thread N: " << strerror(err) << std::endl;
    err = pthread_create(&thread_id_p, NULL, thread_p, (void *) NULL);
    if (err != 0)
        std::cerr << "Can't create thread P: " << strerror(err) << std::endl;
    err = pthread_create(&thread_id_r, NULL, thread_r, (void *) NULL);
    if (err != 0)
        std::cerr << "Can't create thread R: " << strerror(err) << std::endl;
    ...
    sem_close(semaphore_k);
    sem_close(semaphore_m);
    sem_close(semaphore_r);
    ...
    sem_unlink("/my_semaphore_k");
    sem_unlink("/my_semaphore_m");
    sem_unlink("/my_semaphore_r");
    return 0;
}
